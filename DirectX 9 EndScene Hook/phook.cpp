...
void PHook::Init()
{
	while (!init_hook)
	{
		if (GetD3D9Device(pDevice, sizeof(pDevice)))
		{
			...
			{
				MH_CreateHook((LPVOID)(EndSceneAddress), (LPVOID)PHook::hkEndScene, (LPVOID*)&oEndScene);
				MH_EnableHook((LPVOID)(EndSceneAddress));
				oWndProc = (WNDPROC)SetWindowLongPtr(window, GWL_WNDPROC, (LONG_PTR)PHook::WndProc);
				init_hook = true;
				break;
			}
		}
